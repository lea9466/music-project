...
import { convertUser, type CategoryDto, type SongDto, type UserDto } from "../types";
...
function Manage() {
    ...
    const btns = [{ str: 'קטגוריות', icon: 'dataset' }, { str: 'שירים', icon: 'music_note_2' }]
    if (user.role == 2)
        btns.push({ str: 'משתמשים', icon: 'group' })
    const [activeTab, setActiveTab] = useState<string>(btns[0].icon);

    return (
        <>
            <ToggleButtons btns={btns} onSet={setActiveTab} activeTab={activeTab} />
            {activeTab == 'dataset' && <GenericTable elements={categories} displayKeys={catDisplayKeys as (keyof CategoryDto)[]} onDelete={(item: CategoryDto) => onDeletCat(item)} onEdit={(item: CategoryDto) => onEditCat(item)} tableHeaders={catHeaders} buttunAdd={{ text: 'add', function: () => onAddCat() }} />}
            {activeTab == 'music_note_2' && <GenericTable elements={songs} displayKeys={songDisplayKeys as (keyof SongDto)[]} onDelete={onDeletSong} onEdit={(item: SongDto) => onEditSong(item)} tableHeaders={songHeaders} buttunAdd={{ text: 'add', function: () => navigate('/SongController') }} />}
            {activeTab == 'group' && user.role == 2 && <GenericTable elements={users} displayKeys={userDisplayKeys as (keyof UserDto)[]} onDelete={onDeletUser} onEdit={onEditUser} tableHeaders={userHeaders} />}
